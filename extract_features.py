...
class pe_features:
    ...
    IMAGE_DOS_HEADER = ["e_cblp", "e_cp", "e_cparhdr", "e_maxalloc", "e_sp", "e_lfanew"]

    FILE_HEADER = ["NumberOfSections", "CreationYear"] + [
        "FH_char" + str(i) for i in range(15)
    ]
    ...
    def extract_optional_header(self, pe):
        ...
        try:
            OPTIONAL_HEADER_data = [
                ...
                pe.OPTIONAL_HEADER.BaseOfData,  # Check the ImageBase for the condition
                self.Optional_header_ImageBase(
                    pe.OPTIONAL_HEADER.ImageBase
                ),  # Checking for SectionAlignment condition
                self.Optional_header_SectionAlignment(
                    pe.OPTIONAL_HEADER.SectionAlignment,
                    pe.OPTIONAL_HEADER.FileAlignment,
                ),  # Checking for FileAlignment condition
                self.Optional_header_FileAlignment(
                    pe.OPTIONAL_HEADER.SectionAlignment,
                    pe.OPTIONAL_HEADER.FileAlignment,
                ),
                ...
                pe.OPTIONAL_HEADER.MinorSubsystemVersion,  # Checking size of Image
                self.Optional_header_SizeOfImage(
                    pe.OPTIONAL_HEADER.SizeOfImage, pe.OPTIONAL_HEADER.SectionAlignment
                ),  # Checking for size of headers
                self.Optional_header_SizeOfHeaders(
    ...
    def get_count_suspicious_sections(self, pe):
        ...
        for section in pe.sections:
            tmp.append(section.Name.decode("utf-8").split("\x00")[0])
    ...
    def get_text_data_entropy(self, pe):
        result = [0.0, 0.0]
        for section in pe.sections:
            s_name = section.Name.decode("utf-8").split("\x00")[0]
            if s_name == ".text":
...
#     # # source_path = "/home/user/DikeDataset/test-data/"
#     # # output_file = "/home/user/DikeDataset/dataset.csv"
#     # # label = 0
